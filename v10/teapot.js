...
var discardThreshold = 1.8;  // Initial value
var discardThresholdLoc
var hue = 0; 
...
//    gl.cullFace(gl.BACK);
    ...
    discardThresholdLoc = gl.getUniformLocation(program, "discardThreshold");
    ...
    // Event listener for mousewheel
     ...
     window.addEventListener('keydown', function(e) {
        switch(e.key) {
            case 'ArrowLeft':
                hue = (hue - 5) % 360;
                if (hue < 0) hue += 360;  // Keep hue in 0-360 range
                updateDiffuseColor();
                break;
            case 'ArrowUp':
        ...
        discardThreshold = Math.max(0.0, Math.min(4.0, discardThreshold));
    });
...
function updateDiffuseColor() {
    // Convert HSV to RGB (using s=1.0, v=1.0)
    let rgb = hsv2rgb(hue, 1.0, 1.0);
    
    // Update materialDiffuse with new RGB values (keep alpha as 1.0)
    materialDiffuse = vec4(rgb[0], rgb[1], rgb[2], 1.0);
    
    // Recalculate diffuse product
    diffuseProduct = mult(lightDiffuse, materialDiffuse);
...
function render() {
    ...
    gl.uniform1f(discardThresholdLoc, discardThreshold);
